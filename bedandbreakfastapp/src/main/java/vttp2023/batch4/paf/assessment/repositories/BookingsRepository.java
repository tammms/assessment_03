...
public class BookingsRepository {
	...
	public static final String INSERT_NEW_USER = """
			...
			public static final String INSERT_NEW_BOOKING = """
				...
				INSERT INTO booking (booking_id, listing_id, duration, email)
					VALUES
						(?,?,?,?)
	...
	public void newUser(User user) {
		...
		Optional<User> opt = userExists(email);

		if(opt.isEmpty()){
			template.update(INSERT_NEW_USER, user.name(), user.email());
		}
	}
	...
	public void newBookings(Bookings bookings) {
		...
		template.update(INSERT_NEW_BOOKING, bookings.getBookingId(), bookings.getListingId(),
